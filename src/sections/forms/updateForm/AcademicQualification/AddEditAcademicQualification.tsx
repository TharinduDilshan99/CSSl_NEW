...
const AddEditAcademicQualification = ({ AcademicQualifications, onCancel }: Props) => {
    ...
    const AcademicQualificationSchema = yup.object().shape({
        ...
        otherInstituteName: yup.string().when('instituteName', {
            is: 'Other',
            then: () => yup.string().required('Other Institute Name is required'),
            otherwise: () => yup.string().notRequired()
        }),
        qualificationName: yup.string().required('Qualification Name is required'),
        otherqualificationName: yup.string().when('qualificationName', {
            is: 'Other',
            then: () => yup.string().required('Other Qualification Name is required'),
            otherwise: () => yup.string().notRequired()
        }),
        employmentType: yup.string().required('Employment Type is required'),
        ...
        onSubmit: (values, { setSubmitting, resetForm }) => {
            try {
                const submissionData = {
                    actlAcademicQualificationId: values.actlAcademicQualificationId,
                    instituteName: values.instituteName === 'Other' ? values.otherInstituteName : values.instituteName,
                    employmentType: values.employmentType,
                    qualificationName: values.qualificationName === 'Other' ? values.otherqualificationName : values.qualificationName,
                ...
                if (AcademicQualifications) {
                    dispatch(updateAcademicQualification(submissionData));
                } else {
                    dispatch(addAcademicQualification(submissionData));
                }
                resetForm();
                setSubmitting(false);
                onCancel();
                    ...
                    <Form autoComplete="off" noValidate onSubmit={handleSubmit}>
                        ...
                        <DialogContent sx={{ p: 2.5 }}>
                            <Grid container spacing={1}>
                                ...
                                {values.qualificationName === 'Other' && (
                                    <Grid item xs={12} sm={6}>
                                        <Stack spacing={1.25}>
                                            <InputLabel htmlFor="otherqualificationName">Other Qualification Name <span style={{ color: 'red' }}>*</span></InputLabel>
                                            <TextField
