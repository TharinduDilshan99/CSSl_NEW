...
import AddEditProffesionalQualification from 'sections/forms/updateForm/ProffesionalMembership/AddEditProffesionalMembership';
import DeleteProffesionalQualification from 'sections/forms/updateForm/ProffesionalMembership/DeleteProffesionalMembership';
import ProffesionalQualificationView from 'sections/forms/updateForm/ProffesionalMembership/ViewProffesionalMembership';
import { fetchProffesionalQualifications, toInitialState1 } from 'store/reducers/proffesional-membership';
import { queryParamsProps1 } from 'types/proffesional-membership';
...
const ProffesionalQualificationListPage = () => {
    const theme = useTheme();
    const dispatch = useDispatch();
    const { proffesionalQualifications, isLoading, error, success } = useSelector((state) => state.proffesionalQualification);
    ...
    const columns = useMemo(
        () =>
            [
                ...
                {
                    Header: 'Grade / Membership Level',
                    accessor: 'gradeMembership',
                    className: 'cell-center',
                    Cell: ({ value }) => (value ? value : '-')
                },
                {
                    Header: 'Method Of Entry',
                    accessor: 'methodOfEntry',
                    Cell: ({ value }) => (value ? value : '-')
                },
                {
                    Header: 'Address',
                                ...
                                <Tooltip title="Delete">
                                    <IconButton
                                        ...
                                        onClick={(e: MouseEvent<HTMLButtonElement>) => {
                                            ...
                                            const data: VehicleType = row.original;
                                            e.stopPropagation();
                                            setProffesionalQualificationDeleteId(data.proffesionalQualificationId);
                                            setOpen(true);
                                        }}
                                    >
    ...
    const renderRowSubComponent = useCallback(({ row }: { row: Row<{}> }) => <ProffesionalQualificationView data={data[Number(row.id)]} />, [data]);
    ...
    const [ProffesionalQualificationDeleteId, setProffesionalQualificationDeleteId] = useState<any>('');
    ...
    useEffect(() => {
        ...
        dispatch(fetchProffesionalQualifications(queryParams));
    }, [success]);
    ...
    useEffect(() => {
        ...
        if (proffesionalQualifications == null) {
            setData([]);
            return;
        }
        setData(proffesionalQualifications.result!);
    ...
    return (
        <MainCard content={false}>
            ...
            >
                <AddEditProffesionalQualification ProffesionalQualifications={VehicleType} onCancel={handleAddEdit} />
            </Dialog>
            {/* alert model */}
            <DeleteProffesionalQualification title={ProffesionalQualificationDeleteId} open={open} handleClose={handleAlertClose} id={ProffesionalQualificationDeleteId} />
        </MainCard>
    );
};
...
export default ProffesionalQualificationListPage;
